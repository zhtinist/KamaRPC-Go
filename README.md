...
前后端分离：`cmd/console` 是后端，只出 JSON API 并托管前端；前端是原生 HTML/CSS/JS，**无构建步骤、无 npm、无 CDN 依赖**，静态资源用 `go:embed` 编进二进制，部署只有一个可执行文件。

指标有两个来源，**视角不同、都不可少**：

- **服务端** `/stats`（默认 `:9190`/`:9191`）：连接数、协程数、每个方法的调用量与延迟直方图。
- **客户端** `/stats`（`cmd/client` 默认 `:9290`，压测程序用 `-stats` 开）：熔断器三态、连接池水位，以及**被限流或熔断在本地挡掉的调用** —— 这些请求根本没上网络，服务端不可能统计到。客户端记的延迟也是完整往返（含网络与排队），比服务端只量方法执行更接近调用方体感。

面板并发抓取两类端点并与 etcd 拓扑合并。采集侧只记累计值，**QPS 由面板对两次采样做差得出** —— 这样采集侧不必维护时间窗口，也不会因为采集间隔不同而失真。

```bash
etcd &                                          # 注册中心
go run ./cmd/server1 &                          # RPC :9090，指标 :9190
go run ./cmd/server2 &                          # RPC :9091，指标 :9191
go run ./cmd/client &                           # 示例调用方，客户端指标 :9290
go run ./cmd/benchmark_duration -d 600 -stats :9291 &   # 持续压力（可选）
...
- **概览卡片**：总 QPS、错误率、存活实例、etcd 注册实例、活跃连接、协程数、累计调用
- **QPS 折线**：canvas 手绘，没有引图表库
- **服务拓扑**：来自 etcd，实例上下线实时反映（杀掉一个服务端，约 8 秒后租约过期，该实例从列表消失）
- **服务端实例**：连接数 / 协程数 / 运行时长 / PID，抓取失败的实例显示红点与具体错误
- **方法级指标**：按「服务.方法 × 实例」列出 QPS、累计调用、错误率、平均延迟与 P50/P90/P99
- **熔断器**：按「服务@实例 × 调用方」列出三态（闭合 / 熔断 / 半开探测）与窗口内成败计数
- **调用方**：每个客户端进程的连接池水位（已建/上限）、熔断器数量、累计调用

### 故障演示

杀掉 `server2`，面板上会连续发生：

1. 约 1 秒内 —— 客户端熔断器 `Arith@localhost:9091` 变红「熔断」，概览「熔断中」从 0 跳到 1，该实例的连接池水位掉到 `0/1`（死连接被清理）
2. 约 8 秒后 —— etcd 租约（TTL=10s）过期，服务拓扑里该实例消失，服务端实例表显示 `connection refused`
3. 全程总 QPS 不掉 —— 负载均衡把流量全转到存活实例

重启 `server2` 后约 1 秒内自动恢复闭合。半开态只放行一个探测请求、成功即闭合，所以正常恢复时几乎抓不到那一帧（黄色徽章要探测失败或很慢才看得见）。

接口（都是只读，非 GET 一律拒绝）：

| 接口 | 说明 |
| --- | --- |
| `GET /api/overview` | 拓扑 + 各实例指标，面板默认用这个，一次请求拿全 |
| `GET /api/topology` | 只要 etcd 里的服务与实例 |
| `GET /api/stats` | 只要抓来的服务端与客户端指标 |
| `GET /stats`（RPC 服务端上） | 单个服务端自曝的运行状态 |
| `GET /stats`（RPC 客户端上） | 单个调用方的熔断器、连接池与调用指标 |
