...
```
固定时间窗口 50 并发 / 10s，连续 5 次：
  QPS 217563 / 204419 / 198732 / 196175 / 185098   中位数约 199k，失败 0
  P99 0.43 / 0.45 / 0.45 / 0.45 / 0.46 ms
固定请求数 300000 请求 / 100 并发 / batch 100：
  QPS 约 21 万，失败 0（这个口径方差较大，多次实测 11 万~21 万）
```
