...
指标从哪来：每个 RPC 服务端在单独端口上暴露一个**只读**的 `/stats`（默认 `:9190`/`:9191`，`-stats` 可改，置空则不启动），面板并发抓取并与 etcd 里的服务拓扑合并。采集侧只记累计值，**QPS 由面板对两次采样做差得出** —— 这样采集侧不必维护时间窗口，也不会因为采集间隔不同而失真。
...
| `GET /api/stats` | 只要各服务端抓来的指标 |
| `GET /stats`（在 RPC 服务端上） | 单个服务端自曝的运行状态 |
