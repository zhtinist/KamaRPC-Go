...
```
固定请求数   300000 请求 / 100 并发 / batch 100 → QPS 214549, 失败 0
固定时间窗口 50 并发 / 10s → QPS 217054, 失败 0
                              Avg 0.23ms  P50 0.23ms  P90 0.32ms  P99 0.44ms
```

## 性能优化记录

框架的第一个可用版本只有 17k QPS、P99 9.1ms，经过八轮基于 profile 的优化到现在的水平。每轮都有 benchmark 前后对比，`git log` 里记录了完整过程：
...
| 连接池默认值 8 → 2 → 1 | 连接的作用是分散到不同节点，单节点并发靠请求复用即可；实测决定吞吐的是「总连接数」，最优约 2 条，每节点 1 条时总数自然跟随实例数 | 同步 106k → 130k → 217k QPS，P99 0.79 → 0.44ms |
...
| 本实现，默认配置（池=1，与上游一致） | 217054 | 0.44ms |

同配置下吞吐约 **20.5 倍**、P99 约 **22 倍**。上游摘掉限流后只从 9936 涨到 10576，说明限流并非其瓶颈 —— 真正的原因是服务端串行处理叠加每条消息都做 gzip，排队延迟累积到 4.7ms。
