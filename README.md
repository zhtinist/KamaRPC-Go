...
```
固定请求数   300000 请求 / 100 并发 / batch 100 → QPS 149408, 失败 0
固定时间窗口 50 并发 / 10s → QPS 152238, 失败 0
                              P99 0.55ms
```

## 性能优化记录

框架的第一个可用版本只有 17k QPS、P99 9.1ms，经过七轮基于 profile 的优化到现在的水平。每轮都有 benchmark 前后对比，`git log` 里记录了完整过程：
