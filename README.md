...
```
固定请求数   50000 请求 / 100 并发 / batch 100 → QPS 121133, 失败 0
固定时间窗口 50 并发 / 10s → QPS 106529, 失败 0
                              Avg 0.47ms  P50 0.45ms  P90 0.57ms  P99 0.79ms
```

## 性能优化记录

框架的第一个可用版本只有 17k QPS、P99 9.1ms，经过五轮基于 profile 的优化到现在的水平。每轮都有 benchmark 前后对比，`git log` 里记录了完整过程：
...
| 连接池默认值从 8 调到 2 | 连接支持请求复用，一条连接即可承载高并发；连接多了每条上的在途请求变少，系统调用摊不到批量 | 同步 106529 → 130174 QPS，P99 0.79 → 0.58ms |
...
| 本实现，对齐上游（池=1） | 155282 | 0.48ms |
| 本实现，默认配置（池=2） | 130174 | 0.58ms |

同配置下吞吐约 14.6 倍、P99 约 20 倍。上游摘掉限流后只从 9936 涨到 10576，说明限流并非其瓶颈 —— 真正的原因是服务端串行处理叠加每条消息都做 gzip，排队延迟累积到 4.7ms。
